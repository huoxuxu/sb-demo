 ...
 * @Author: huoxuxu
 * @Description:
 * @Date: 2023-10-09 14:53:19
 **/
public class TitansHttpClientUtils {
    private TitansHttpClientUtils() {
        throw new IllegalStateException("Utility class");
    }

    private static final Logger logger = LoggerFactory.getLogger(TitansHttpClientUtils.class);
    ...
    static {
        ...
        Set<String> httpInterceptors = new HashSet<>();
        //PluginConfigManager.getPropertyValueSet("com.zto.titans.soa.http.interceptor.HttpInterceptor");
        if (httpInterceptors != null && !httpInterceptors.isEmpty()) {
            for (String interceptor : httpInterceptors) {
                try {
                    ...
                    throw new IllegalArgumentException("Http加载插件异常,请检查配置文件", e);
                }
            }
        }
...
//        //获取或者创建filterChain对象
//        HttpClientFilterChain httpClientFilterChain = HttpClientUtils.httpClientFilterChainThreadLocal.get();
//
...
//            HttpClientUtils.httpClientFilterChainThreadLocal.set(httpClientFilterChain);
//        }
